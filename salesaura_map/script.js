...
// ===== LOAD DATA =====
Papa.parse(sheetCSV + "&v=" + Date.now(), {
  download: true,
  skipEmptyLines: true,
  complete: res => {
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      if (isNaN(d.lat) || isNaN(d.lng)) continue;
...
function applyFilter() {
  ...
  allData.forEach(d => {
    ...
    if (okSales && okFrom && okTo) {
      d.marker.addTo(markerLayer);
      bounds.push([d.lat, d.lng]);
      stats[d.sales] = (stats[d.sales] || 0) + 1;
      filteredData.push(d);
    } else {
      markerLayer.removeLayer(d.marker);
    }
  });
...
// (bagian PDF kamu BIARKAN SAMA persis)
